...
// bind: 특정 Ip와 연결.
// my_addr에 있는 서버의 소켓 주소를 소켓 식별자 sockfd에 연결
...
// 소켓 주소 serv_addr의 서버와 인터넷 연결을 시도.
// connect 함수는 연결이 성공할 때까지 블록되어 있거나 에러 발생.
// 연결이 성공하면 sockfd 식별자는 읽거나 쓸 준비가 됌.
...
{
    size_t nleft = n; // 남은 바이트 수 저장
    ssize_t nwritten; // 
    char *bufp = usrbuf;

    while (nleft > 0) // 남은 바이트 수가 0이 될때까지 반복
    {
...
{
    rp->rio_fd = fd;              // rio 구조체의 파일 디스크립터를 설정
    rp->rio_cnt = 0;              // rio_cnt: 현재 버퍼에 읽어들인 바이트 수
    rp->rio_bufptr = rp->rio_buf; // 다음에 읽을 데이터 위치를 가르킴
}
/* $end rio_readinitb */
...
{
    int n, rc;              // rio_read return == rc // n은 현재까지 읽은 데이터의 길이
    char c, *bufp = usrbuf; // bufp : 사용자가 제공한 버퍼의 시작 주소를 가리킴

    for (n = 1; n < maxlen; n++)
    { // 버퍼에 담긴 모든 데이터 읽기
        // rio_read의 return값은 1. 버퍼에 담긴 바이트 수. 2. EOF에 도달하여 읽을 데이터가 없는 경우 0 3. -1 읽기 중 오류 발생
        if ((rc = rio_read(rp, &c, 1)) == 1)
        { // rio_read가 성공적으로 1바이트를 읽어온 경우 1 반환
            *bufp++ = c;
            if (c == '\n')
            { // \n
                n++;
        ...
        else if (rc == 0) // EOF에 도달하여 읽을 데이터가 없는 경우
        { 
            if (n == 1) 
...
int open_clientfd(char *hostname, char *port)
{
    ...
    hints.ai_socktype = SOCK_STREAM; // TCP 방식으로 연결
    hints.ai_flags = AI_NUMERICSERV; // 포트 번호를 숫자 형태로 지정
    hints.ai_flags |= AI_ADDRCONFIG; // 주소 구성 정보가 없는 경우 대비
